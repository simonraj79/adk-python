"""Level 4a — Self-Evolving System over a domain-specific MCP data source.

Variant of `level_4_agent` where `data_fetcher_agent` swaps the `google_search`
built-in for an `McpToolset` pointed at the gahmen-mcp Singapore Government
Data MCP server (data.gov.sg + SingStat Table Builder).

Everything else — coordinator meta-reasoning, BuiltInCodeExecutor analyst,
agent_creator, state.capabilities re-hydration, safety allowlist — is
structurally identical to Level 4. See AGENT_LEVELS.md "Level 4a".
"""
...
# so it can be safely wrapped as an AgentTool.
# ---------------------------------------------------------------------------
...
data_fetcher_agent = Agent(
    ...
    description=(
        "Fetches Singapore government data: data.gov.sg datasets (CKAN "
        "datastore search + metadata) and SingStat tables (time series + "
        "cross-sectional). Returns raw facts; does not compute."
    ),
    instruction=(
        "You are a Singapore government data research specialist. Prefer"
        " SingStat tools for official macro/demographic time series, and"
        " data.gov.sg tools for granular operational datasets. Return"
        " structured findings (dataset id, columns, sample rows, source"
        " URL). Never compute or interpret — that is the analyst's job."
    ),
    tools=[gahmen_toolset],
...
analyst_agent = Agent(
    name="analyst_agent",
    # Flash + BuiltInPlanner is the AGENTS.md-sanctioned "reasoning model"
    # knob for code-exec leaves. Gotcha #21 forbids Pro on
    # BuiltInCodeExecutor agents (Pro + code_execution + AFC has been
    # observed to hang 6+ min on multi-step numeric+chart prompts). The
    # planner switches Gemini's native thinking ON for Flash with
    # include_thoughts=True so the analyst plans the cell layout BEFORE
    # writing code — which directly addresses the multi-cell drift that
    # produces blank artifacts (gotcha #20: "if a later code cell
    # closes/re-saves the figure, Gemini will save a second blank
    ...
    description=(
        "Business analyst. Executes Python for every numeric result (CAGR, "
        "percent change, weighted averages, ratios, pandas exploration) and "
        "ALWAYS produces at least one matplotlib chart for time-series, "
        "comparative, or multi-category inputs. Returns a text summary; "
        "charts surface in adk web via the framework's auto-save of "
        "BuiltInCodeExecutor image output into the artifact service."
    ),
    instruction="""You are a business analyst backed by Gemini's code_execution sandbox.

# Mandatory behaviour
1. Never mental-math. Every numeric result comes from `print(...)` inside executed Python.
2. You MUST produce at least one matplotlib chart whenever the input is a time series, a comparison across categories, or more than two numeric rows. A line plot for trends, a bar plot for categorical comparisons. Only skip the chart if the input is a single scalar or the user explicitly says "no chart".
3. Print numeric summaries with f-strings so the coordinator can quote them verbatim.

# Charting rules — follow EXACTLY
The chart MUST be produced in ONE single code cell. Splitting figure construction and `plt.show()` across cells is the #1 cause of blank PNGs in this sandbox: the figure created in cell A is closed by the time `plt.show()` runs in cell B, so the framework auto-saves an empty canvas as the artifact.
...
- NEVER call `save_artifact` or reference `tool_context` — they don't exist inside the sandbox.
...
# ---------------------------------------------------------------------------
# Agent creator — same pattern as L4. Not an AgentTool (creation is a
...
# Workflow
1. Read the mission the coordinator gave you.
2. Draft a spec out loud:
   - name: snake_case identifier (e.g., "sg_labour_agent")
   ...
   - tool_set: a list of ALLOWED tool names (see safety.py allowlist — includes
     google_search AND the gahmen-mcp tools datagovsg_* / singstat_*)
...
# Rules
- Use ONLY tool names from the safety allowlist. Violations are rejected.
...
_FIXED_TOOLS = [
    ...
    # Do NOT set skip_summarization=True here — it ends the coordinator's turn
    # at the function_response without emitting the writer's BI brief as a
    # visible text event. Allowing summarization costs one extra LLM call but
    # makes the final brief appear as a chat bubble in adk web.
    AgentTool(agent=report_writer_agent),
]


def _rehydrate_runtime_tools(callback_context: CallbackContext):
  """Rebuild tools = fixed + session-specific runtime tools on each turn.

  Same pattern as level_4_agent: rebuild from scratch (not append) so a
...
root_agent = Agent(
    name="level_4a_agent",
    model="gemini-2.5-pro",  # Pro for meta-reasoning / capability-gap detection
    description=(
        "Self-evolving Singapore Government Business Intelligence coordinator."
        " Routes analytical questions to fixed specialists (data_fetcher,"
        " analyst, report_writer) using data.gov.sg and SingStat via MCP,"
        " and creates new specialists on demand when a capability gap is"
        " detected."
    ),
    instruction="""You are a Singapore Government data BI coordinator. You delegate all work — you do not compute, search, or format yourself.

# Your team (call as AgentTools)
- `data_fetcher_agent` — queries Singapore government data (data.gov.sg + SingStat via MCP)
...
- If NO (user needs a capability none of my specialists cover — e.g., data
  from a different country, live CRM, non-government SG data): transfer to
...
3. Quote analyst_agent's printed numbers VERBATIM. Do not round, re-format, or
   re-interpret them.
4. When you delegate to analyst_agent, your request MUST instruct it to
   produce at least one matplotlib chart unless the answer is a single scalar.
   The chart is how the user sees trends — text alone is not enough.
5. After report_writer_agent returns, your final reply to the user MUST be the
   writer's BI brief reproduced verbatim (do not paraphrase, truncate, or add
   framing). This is how the user sees the brief.
6. For simple factual or metadata questions one data_fetcher_agent call may be
   enough — skip the analyst and writer in that case.
""",
